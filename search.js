...
var apigatewayendpoint = 'https://gt0ytth3cc.execute-api.us-east-2.amazonaws.com/opensearch-api-test?q=us-east-2';
  ...
  if (query.length > 2) {
    ...
    if (results.length > 0) {
      ...
      for (var item in results) {
        ...
        let type = results[item]._source.content;
        let region = results[item]._source.awsRegion;
        ...
        '<a href="' + url + '"></a>' +
        '<div><h2><a href="' + url + '">' + type + '</a></h2><p>' + year + ' &mdash; ' + plot + '</p></div></div>');
      }
    } else {
      noresults.show();
    }
